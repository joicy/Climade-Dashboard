...
def main():
    st.set_page_config(
        page_title="Dengue Virus 4 Africa Dashboard",
        layout="wide",
    ...
    main_title( "DENGUE VIRUS 4 - AFRICA DASHBOARD" ,display_countries)
    ...
    if map_option == 'Total number of genomes':
        ...
        voc_selected = c1.selectbox("Choose genotype to show", dengue_four)
        df_count_map = sd.new_build_df_count(df_africa[df_africa['lineage'] == voc_selected], True)
        colorpath_africa_map(df_count_map, column=c1, color_pallet=dengue_four_color.get(voc_selected))
